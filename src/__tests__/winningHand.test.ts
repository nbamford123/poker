...
test('finds winning straight', () => {
  expect(winningHand(straightHand2, straightHand)).toBe(WinningHand.SecondHand);
});
...
test('finds winning two pair 2', () => {
  expect(winningHand(twoPairHand2, twoPairHand3)).toBe(WinningHand.SecondHand);
});
test('finds winning two pair high card', () => {
  const twoPairHand4 = {...twoPairHand, extra: [{number: 10, suit: Suit.Club}]};
...
test('finds winning high card', () => {
  expect(winningHand(highCardHand, highCardHand2)).toBe(WinningHand.FirstHand);
});
